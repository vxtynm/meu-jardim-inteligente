/* script.js - identificação integrada (Plant.id optional) + fallback público (Wikipedia search)
   Instruções:
   - Para usar Plant.id: crie uma conta e cole sua chave em API_KEY.
   - O código tenta: Plant.id (se configurado) -> heurística local -> fallback Wikipedia.
*/
...
function renderCatalog(){ plantsGrid.innerHTML = PLANT_CATALOG.map(p=>`
  <div class="plant-card">
    <img src="${p.img}" alt="${p.name}">
    <h3>${p.name}</h3>
    <div class="plant-info">🌿 Duração: ${p.life}<br>💧 Regar: ${p.water}<br>☀️ Luz: ${p.light}<br><small>${p.tips}</small></div>
  </div>
`).join(''); }

function fileToDataUrl(file){ return new Promise((res,rej)=>{ const reader = new FileReader(); reader.onload = ()=>res(reader.result.split(',')[1]); reader.onerror = e=>rej(e); reader.readAsDataURL(file); }); }

async function identifyWithPlantId(file){
  try{
    const b64 = await fileToDataUrl(file);
    const body = {images:[b64],modifiers:["crops_fast","similar_images"],plant_language:'pt'};
    const resp = await fetch('https://api.plant.id/v2/identify',{method:'POST',headers:{'Content-Type':'application/json','Api-Key':API_KEY},body:JSON.stringify(body)});
    ...
    return null;
  }catch(err){ console.error('Plant.id erro',err); return null; }
}
...
function identifySimulated(file){
  ...
  for(const w of words){ const m = PLANT_CATALOG.find(p=> p.name.toLowerCase().includes(w) || p.id.includes(w)); if(m) return {name:m.name,source:'simulado',info:m}; }
  return null;
}
...
async function fallbackWikipedia(query){
  try{
    ...
    const url = `https://pt.wikipedia.org/w/api.php?action=query&list=search&srsearch=${q}&format=json&origin=*`;
    const resp = await fetch(url);
    if(!resp.ok) return null;
    const data = await resp.json();
    if(data.query && data.query.search && data.query.search.length){
      return data.query.search.slice(0,3).map(s=>({title:s.title,snippet:s.snippet}));
    }
    return null;
  }catch(e){ console.error('Wiki err',e); return null; }
}
...
async function identifyPlant(file){
  ...
  if(USE_PLANT_ID){
    ...
    if(res){ identifyResult.innerHTML = `<h3>${res.name}</h3><p>Confiabilidade: ${Math.round((res.score||0)*100)}%</p>`; return; }
  }
  ...
  const wiki = await fallbackWikipedia(filename || 'planta');
  if(wiki && wiki.length){
    identifyResult.innerHTML = `<h3>Possíveis correspondências (busca pública)</h3>` + wiki.map(w=>`<div style="margin-bottom:8px"><strong>${w.title}</strong><div class="small">${w.snippet}</div></div>`).join('');
    return;
  }
  identifyResult.innerHTML = `<h3>Não foi possível identificar automaticamente</h3><p class="muted">Sugestões do catálogo:</p>` + PLANT_CATALOG.map(p=>`<div class="plant-card small" style="display:inline-block;width:150px;margin:6px;padding:8px;"><img src="${p.img}" style="height:80px;width:100%;object-fit:cover;border-radius:6px"><strong>${p.name}</strong><div class="small">${p.life}</div></div>`).join('');
}

document.getElementById('identifyBtn').addEventListener('click', async ()=>{
  const input = document.getElementById('photoInput');
  if(!input.files || !input.files[0]){ alert('Selecione uma foto'); return; }
  const file = input.files[0];
  await identifyPlant(file);
});

function renderLog(){
  if(!registros.length){ logDiv.innerHTML = '<p class="muted">Nenhum registro ainda.</p>'; return; }
  logDiv.innerHTML = registros.map((r,i)=>`
    <div class="entry"><div><strong>${r.text}</strong><div class="small">${r.t}</div></div>
    <div><button onclick="deleteRecord(${i})">🗑</button></div></div>
  `).join('');
}
window.deleteRecord = function(i){ if(!confirm('Apagar registro?')) return; registros.splice(i,1); localStorage.setItem('registros',JSON.stringify(registros)); renderLog(); renderChart(); }
...
document.getElementById('addRecord').addEventListener('click', ()=>{
  ...
  document.getElementById('plantName').value='';document.getElementById('time').value='';
});
...
function renderChart(){
  ...
  chart = new Chart(chartCtx,{type:'doughnut',data:{labels:['Regas','Adubos','Podas'],datasets:[{data:[regas,adubos,podas],backgroundColor:['#43a047','#8d6e63','#ffb300']}]},options:{responsive:true}});
}
...
renderCatalog(); renderLife(); renderLog(); renderChart();
